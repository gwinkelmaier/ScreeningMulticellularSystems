...
model = torch.load('trained-models/model-region.net')
print('MODEL LOADED')
...
matio.save(fileDir .. 'data/Mina/inputs/train-output-region' .. t .. '.mat', out1)
end
...
for t = 1 , 496, 1 do
   ...
   y = model:forward(x)
   out1 = torch.DoubleTensor(2,224,224):copy(torch.squeeze(y[{1,{},{},{}}]))
   matio.save(fileDir .. 'data/Mina/inputs/train-output-region' .. t .. '_' .. letters[j] .. '.mat', out1)
   end
end
...
y = model:forward(x)
out1 = torch.DoubleTensor(2,224,224):copy(torch.squeeze(y[{1,{},{},{}}]))
matio.save(fileDir .. 'data/Mina/inputs/test-output-region' .. t .. '.mat', out1)
end
...
for t = 1 , 16, 1 do
   ...
   y = model:forward(x)
   out1 = torch.DoubleTensor(2,224,224):copy(torch.squeeze(y[{1,{},{},{}}]))
   matio.save(fileDir .. 'data/Mina/inputs/test-output-region' .. t .. '_' .. letters[j] .. '.mat', out1)
   end
end
