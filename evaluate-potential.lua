...
model = torch.load('trained-models/model-potential.net')
print('MODEL LOADED')
...
x = x:cuda()
y = model:forward(x)
out1 = torch.DoubleTensor(1,224,224):copy(torch.squeeze(y[{1,{},{},{}}]))
matio.save(fileDir .. 'data/Mina/inputs/train-output-potential' .. t .. '.mat', out1)
end
...
for t = 1 , 496, 1 do
   ...
   y = model:forward(x)
   out1 = torch.DoubleTensor(1,224,224):copy(torch.squeeze(y[{1,{},{},{}}]))
   matio.save(fileDir .. 'data/Mina/inputs/train-output-potential' .. t .. '_' .. letters[j] .. '.mat', out1)
   end
end
...
y = model:forward(x)
out1 = torch.DoubleTensor(1,224,224):copy(torch.squeeze(y[{1,{},{},{}}]))
matio.save(fileDir .. 'data/Mina/inputs/test-output-potential' .. t .. '.mat', out1)
end
...
for t = 1 , 16, 1 do
   ...
   y = model:forward(x)
   out1 = torch.DoubleTensor(1,224,224):copy(torch.squeeze(y[{1,{},{},{}}]))
   matio.save(fileDir .. 'data/Mina/inputs/test-output-potential' .. t .. '_' .. letters[j] .. '.mat', out1)
   end
end
